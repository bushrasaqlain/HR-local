...
const Avatar = ({ src, name, size = 40, locked = false }) => {
    ...
    return (
      <div
        ...
        style={{
          width: size,
          height: size,
          background: "linear-gradient(135deg, #5f8190, #36565f)",
  ...
  return (
    <img
      src={
        src
          ? `${process.env.NEXT_PUBLIC_API_BASE_URL}${src.replace(/^\/+/, "")}`
          : "/images/user.png"
      }
      alt={name}
      onError={(e) => {
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`badge rounded-pill ${map[status] || "bg-secondary text-white"}`}
    >
      {status || "Pending"}
    </span>
  );
...
const TierBadge = ({ candidate }) => {
  ...
  return (
    <span
      className={`badge rounded-pill ${map[candidate.tier] || "bg-secondary-subtle text-secondary"}`}
      style={{ fontSize: 11 }}
    >
...
const CandidateMeta = ({ candidate }) => {
  const specName =
    ...
    (candidate.speciality_name !== "-" ? candidate.speciality_name : null);
  const skillNames =
  ...
  return (
    ...
    >
      {specName && (
        <span style={{ color: "#805ad5", fontWeight: 600 }}>{specName}</span>
      )}
      {skillNames.length > 0 && (
...
class AllApplicants extends Component {
  ...
  openCandidatePage = (candidate) => {
    ...
    this.setState({ selectedCandidate: null }, () => {
      this.setState({
        ...
        splitViewActive: !mobile,
        mobileDetailView: mobile,
  ...
  viewScreeningAnswers = async (candidate) => {
    ...
    this.setState({
      screeningAnswersOpen: true,
      loadingAnswers: true,
      screeningAnswers: null,
    });
    try {
      const res = await axios.get(
        `${this.apiBaseUrl}applications/${candidate.application_id}/answers`,
      );
      this.setState({
        screeningAnswers: res.data.answers,
        loadingAnswers: false,
      });
    } catch (err) {
      console.error(err);
  ...
  closeScreeningAnswers = () =>
    this.setState({ screeningAnswersOpen: false, screeningAnswers: null });
  ...
  fetchPostedJobs = async () => {
    try {
      const res = await axios.get(
        `${this.apiBaseUrl}job/managejob/${this.userId}`,
      );
  ...
  fetchAllCandidates = async () => {
    ...
    if (!selectedJobId)
      return this.setState({ candidates: [], allApplicants: [] });
    ...
    try {
      ...
      const candidates = (res.data.candidate || []).map((c) => {
        const otherPreferredCities = (c.otherPreferredCities || []).map(
          (city) =>
            typeof city === "number"
              ? { id: city, name: cityMapObj[city] || "" }
              : { id: city.id, name: city.name || cityMapObj[city.id] || "" },
        );
        const mainCityMatch = Number(c.city) === Number(jobCityId);
        const preferredMatch = otherPreferredCities.find(
          (city) => Number(city.id) === Number(jobCityId),
        );
        const city_name = mainCityMatch
      ...
      candidates.forEach((a) => {
        ...
        if (
          a.has_applied &&
          (!a.candidateStatus || a.candidateStatus === "Pending")
        )
          counts.applied++;
      });
      ...
      this.setState({
        ...
        allApplicants: [],
        jobMessage:
          apiError === "Job not found" || apiError === "Job is pending approval"
  ...
  unlockCandidate = async (candidateId) => {
    this.setState((prev) => {
      const s = new Set(prev.unlockingIds);
      s.add(candidateId);
      return { unlockingIds: s };
    });
    try {
      const res = await axios.post(
        `${this.apiBaseUrl}applicant/unlock-candidate`,
        { candidateId, jobId: this.state.selectedJobId },
      );
      ...
      this.setState((prev) => ({
        ...
        budgetStatus: budget_status
          ? { ...prev.budgetStatus, ...budget_status }
          : prev.budgetStatus,
      }));
    } catch (error) {
  ...
  handleApplicationStatus = async (
    ...
    try {
      ...
      if (this.state.selectedCandidate?.candidate_id === candidateId) {
        const updated = this.state.allApplicants.find(
    ...
    return allApplicants.filter((c) => {
      const tabMatch =
        activeTab === "applicants"
          ? c.has_applied === true &&
            (!c.candidateStatus || c.candidateStatus === "Pending")
      ...
      const statusMatch =
        activeTab === "applicants" || activeTab === "all"
          ? true
          : selectedStatus
            ? String(c.candidateStatus || "")
                .trim()
                .toLowerCase() === selectedStatus.toLowerCase()
            : true;

      const cityMatch = selectedCityId
        ? Number(c.city) === Number(selectedCityId) ||
          c.otherPreferredCities?.some(
  ...
  renderBudgetBanner() {
    ...
    const pct =
      daily_cap > 0
        ? Math.min(100, Math.round((spent_today / daily_cap) * 100))
        : 0;
    return (
      <div
        className={`alert ${is_exhausted ? "alert-danger" : "alert-info"} mb-3`}
      >
        ...
        <div className="progress mb-1" style={{ height: 6 }}>
          <div
            className={`progress-bar ${is_exhausted ? "bg-danger" : ""}`}
            style={{ width: `${pct}%` }}
          />
        </div>
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            ...
            <div className="bg-white rounded-3 shadow p-3 p-md-4 mb-3">
              ...
              <div className="position-relative">
                <input
                  ...
                  onFocus={() =>
                    this.setState({ showJobDropdown: true, isTyping: false })
                  }
                  onChange={(e) =>
                    this.setState({
                      jobSearch: e.target.value,
                  ...
                  onBlur={() =>
                    setTimeout(() => {
                      const selectedJob = this.state.postedJobs.find(
                        (j) => j.id === this.state.selectedJobId,
                      );
                      this.setState({
                        showJobDropdown: false,
                        isTyping: false,
                  ...
                  onBlur={() =>
                    setTimeout(
                      () => this.setState({ showJobDropdown: false }),
                      150,
                    )
                  }
                  style={{
                ...
                {this.state.showJobDropdown && (
                  ...
                  >
                      ...
                      .filter(
                        (j) =>
                          ...
                          j.job_title
                            .toLowerCase()
                            .includes(
                              (this.state.jobSearch || "").toLowerCase(),
                            ),
                      )
                      .map((j) => (
                        <div
                          ...
                          style={{
                            cursor: "pointer",
                            background:
                              this.state.selectedJobId === j.id
                                ? "#e8f0f1"
                                : "",
                            color: "#2d3748",
                            fontWeight:
                              this.state.selectedJobId === j.id ? 600 : 400,
                          }}
                          onMouseEnter={(e) =>
                            (e.currentTarget.style.background = "#e8f0f1")
                          }
                          onMouseLeave={(e) =>
                    ...
                    {this.state.postedJobs.filter((j) =>
                      j.job_title
                        .toLowerCase()
                        .includes((this.state.jobSearch || "").toLowerCase()),
                    ).length === 0 && (
            ...
            <div className="d-flex gap-2 mb-3">
              {[
                ...
                {
                  key: "applicants",
                  label: `Applicants (${counts.applied || 0})`,
                },
                ...
                <button
                  ...
                  onClick={() =>
                    this.setState({ activeTab: tab.key, currentPage: 1 })
                  }
                >
                  {tab.label}
                </button>
            ...
            {this.state.jobMessage && (
              <div className="alert alert-warning">{this.state.jobMessage}</div>
            )}
              ...
              <>
                ...
                {showSplit && (
                  <div className="d-flex gap-3">
                    <div
                      className="bg-white rounded-3 shadow p-3 overflow-auto"
                      style={{
                        flex: "0 0 28%",
                          ...
                          return (
                            <div
                              ...
                              onClick={() => {
                                ...
                                !isLocked && this.openCandidatePage(c);
                              }}
                            >
                              <Avatar
                    ...
                    <div
                      className="bg-white rounded-3 shadow p-3 position-relative overflow-auto"
                      style={{ flex: 1, maxHeight: "calc(100vh - 300px)" }}
                    >
                      <button
                  ...
                  <div className="bg-white rounded-3 shadow p-3">
                    {current.length === 0 ? (
                      <div className="text-center py-5 text-muted">
                        ...
                        <p className="small">
                          Select a job above to view matching candidates
                        </p>
                      </div>
                    ) : (
                      <>
                            ...
                            <thead>
                              <tr style={{ background: BRAND }}>
                                {[
                                  ...
                                  <th
                                    key={h}
                                    className="text-white fw-semibold small py-3 text-center"
                                  >
                                    {h}
                                  </th>
                                ))}
                                  ...
                                  <tr
                                    ...
                                    style={{
                                      ...
                                      borderLeft: isSelected
                                        ? `4px solid ${BRAND}`
                                        : !isLocked && c.is_boosted
                                          ? "3px solid #f59e0b"
                                          : "",
                                      background:
                                        !isLocked && c.is_boosted && !isSelected
                                          ? "#fffbeb"
                                          : undefined,
                                    }}
                                    onClick={() => {
