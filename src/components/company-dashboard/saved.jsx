...
const StatusBadge = ({ status }) => {
  ...
  return <span className={`badge rounded-pill ${map[status] || "bg-secondary text-white"}`}>{status || "Saved"}</span>;
};
...
class Saved extends Component {
  state = {
    ...
    selectedStatus: "Saved", selectedCityId: "", searchFilters: {},
  ...
  fetchAllCandidates = async () => {
    ...
    try {
      ...
      candidates.forEach(a => {
        ...
        if (a.candidateStatus === "Saved") counts.shortlisted++;
        if (a.candidateStatus === "Rejected") counts.rejected++;
        if (a.candidateStatus === "Approved") counts.approved++;
      });

      this.setState({
        candidates, allApplicants: candidates, counts, selectedStatus: "Saved",
        splitViewActive: false, mobileDetailView: false,
        selectedCandidate: null, selectedCandidateId: null,
      });
    } catch (err) { console.error(err); }
  ...
  render() {
    ...
    return (
      <>
        <Head><title>Saved Candidates</title></Head>
                  ...
                  <div className="bg-white rounded-3 shadow p-3">
                    {current.length === 0 ? (
                      <div className="text-center py-5 text-muted">
                        <p className="fw-semibold mb-1">No saved candidates found</p>
