...
const SkeletonCard = () => (
  <div className="card p-3">
    <div className="placeholder-glow">
      <div
        className="rounded-circle placeholder bg-secondary mb-3"
...
const CandidateCard = ({
  ...
  return (
    <div
      className={`card h-100 ${candidate.is_boosted ? "border-warning" : ""}`}
    >
      ...
      <div className="card-body d-flex flex-column gap-2">
        ...
        <div className="text-muted small">
          📍 {candidate.city_name || "Location not specified"}
        </div>

        <div className="d-flex flex-wrap gap-1">
          <span className="badge bg-light text-secondary border">
            ⏰ {candidate.availability_status || "Not set"}
          </span>
          <span className="badge bg-light text-secondary border">
            🛠️ {candidate.skills_count || 0} skills
          </span>
          {candidate.total_experience && (
            <span className="badge bg-light text-secondary border">
              💼 {expLabel(candidate.total_experience)}
            </span>
          )}
        </div>
        ...
        {!unlockedData && !candidate.hasActiveJob && (
          <div className="text-warning small mt-auto">
            ⚠️ Post a job to unlock
          </div>
        )}
      </div>
    </div>
...
class AvailableCandidates extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({ hasSearched: true }, () => this.fetchCandidates());
  }
  fetchCities = async () => {
    try {
      const res = await axios.get(`${this.apiBase}getallCities`, {
        params: { limit: 5000 }, // large enough to cover all cities
      });
  ...
  checkEmployerActiveJob = async () => {
    try {
      const res = await axios.get(
        `${this.apiBase}job/managejob/${this.userId}`,
      );
      const hasActiveJob = (res.data || []).some(
  ...
  fetchCandidates = async (overrides = {}) => {
    const { page, limit, search, cityId, experience, skillId } = {
      ...this.state,
      ...overrides,
    };
    if (
  ...
  fetchUnlockedCandidates = async () => {
    try {
      const res = await axios.get(`${this.apiBase}credit/unlocked-list`, {
        headers: { Authorization: `Bearer ${this.token}` },
      });
      ...
      await Promise.all(
        list.map(async ({ candidate_id }) => {
          try {
            const r = await axios.get(
              `${this.apiBase}credit/profile/${candidate_id}`,
              { headers: { Authorization: `Bearer ${this.token}` } },
            );
            unlockedIds.add(candidate_id);
            unlockedData[candidate_id] = {
              scope: r.data.scope,
              data: r.data.data,
            };
          } catch (err) {
  ...
  handleUnlock = async (candidateId) => {
    ...
    try {
      ...
      if (!unlockRes.data.already_unlocked)
        this.showAlert("success", "Profile unlocked!");
    } catch (err) {
      const msg =
  ...
  render() {
    ...
    return (
      <>
        <Head>
          <title>Available Candidates | Dashboard</title>
        </Head>
        ...
        >
          ...
          {alertMsg && (
            <div
              className={`alert alert-${alertMsg.type === "success" ? "success" : "danger"} d-flex align-items-center gap-2`}
            >
          ...
          <div className="d-flex justify-content-between align-items-start flex-wrap gap-3 mb-4">
            <div>
              ...
              <p className="text-muted small mb-0">
                Browse candidates — post a job &amp; buy CV Credits to unlock
                profiles
              </p>
            </div>
            {creditBalance && (
              <div className="d-flex align-items-center gap-2 border rounded-3 px-3 py-2 bg-light">
                ...
                <div>
                  ...
                  <div className="text-muted" style={{ fontSize: 11 }}>
                    {creditBalance.package_name} · {creditBalance.unlock_scope}{" "}
                    scope
                  </div>
                </div>
              </div>
              ...
              <div>
                <strong>
                  {(stats?.total_candidates || total) > 0
                    ? `${stats?.total_candidates || total}+ candidates ready to hire!`
                    : "Candidates are ready to hire!"}
                </strong>
                <div className="small">
                  Post a job and buy a CV Credits package to unlock full
                  profiles
                </div>
              </div>
              <button
                className="btn btn-success btn-sm"
                onClick={this.goToPostJob}
              >
                Post a Job →
              </button>
            </div>
          ...
          {hasActiveJob && !creditBalance && (
            <div
              className="alert d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4"
              style={{ background: "#e2f0f0", borderColor: "#e2f0f0" }}
            >
              <div>
                <strong style={{ color: "#36565f" }}>
                  Your job is live! Buy a CV Credits package to unlock profiles.
                </strong>
                <div className="small" style={{ color: "#36565f" }}>
                  Each credit unlocks one candidate's profile.
                </div>
              </div>
              <button
          ...
          {hasActiveJob && creditBalance && (
            ...
            >
              <strong>Ready to hire!</strong> Click "Unlock" on any candidate to
              reveal their profile. You have{" "}
              <strong>{creditBalance.remaining_credits}</strong> credits (
              {creditBalance.unlock_scope} scope).
            </div>
          )}
          ...
          {loading ? (
            ...
            <div className="text-center py-5 text-muted">
              ...
              <p className="small">
                Use the filters above to find candidates by skill, experience,
                or city
              </p>
              <div className="d-flex flex-wrap justify-content-center gap-2 mt-3">
                {["fresh", "1-3", "3-5", "5+"].map((exp) => (
                  <button
                    ...
                    onClick={() => this.handleFilter("experience", exp)}
                  >
                    {exp === "fresh"
                      ? "🎓 Fresh"
                      : exp === "1-3"
          ...
          {/* Bottom CTA */}
          <div
            className="text-center rounded-4 p-4 mt-4 border"
            style={{ background: "#e2f0f0", borderColor: "#e2f0f0" }}
          >
            <h6 className="fw-semibold" style={{ color: "#36565f" }}>
              Want to hire from these candidates?
            </h6>
            <p className="small mb-3" style={{ color: "#000" }}>
              Post a job, choose a CV Credits package, and start unlocking
