...
const CandidateCard = ({ candidate, index, isLocked }) => {
        ...
        >
            ...
            <div style={styles.cardSub}>
                {isLocked ? (
                    ...
                    <>
                        {expLabel(candidate.total_experience)} experience
            ...
            {isLocked && (
                <div style={styles.lockOverlay}>
                    ...
                    <p style={styles.lockLabel}>Post a job to unlock</p>
                </div>
            )}
        </div>
    );
...
class AvailableCandidates extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            experience: "",
            cities: [],
            hasActiveJob: false,   // does this employer have an approved active job?
            checkingJob: true,
        };
    }
    ...
    // ── Check if employer has at least one approved + active job ─────────────────
    ...
    // ── Fetch city list for filter dropdown ──────────────────────────────────────
    fetchCities = async () => {
        try {
    ...
    goToPostJob = () => {
        if (this.props.onTabChange) {
            this.props.onTabChange("postJob");
        }
    };

    goToPackages = () => {
        if (this.props.onTabChange) {
            this.props.onTabChange("viewpackage");
        }
    };
    ...
    render() {
        const {
            ...
            search, cityId, experience, cities,
            hasActiveJob, checkingJob,
        } = this.state;

        const totalPages = Math.ceil(total / limit);

        // First 2 cards are visible (teaser), rest are locked
        const VISIBLE_FREE = 2;
                ...
                <div style={styles.page}>
                    ...
                    <div style={styles.pageHeader}>
                        <div>
                            ...
                            <p style={styles.pageSub}>
                                Browse all registered candidates — post a job &amp; buy a package to access full profiles
                            </p>
                        </div>
                    </div>
                            ...
                            <div style={styles.ctaBannerLeft}>
                                ...
                                <div>
                                    ...
                                    <p style={styles.ctaBannerSub}>
                                        Post a job and buy a package to view full profiles, contact details &amp; schedule interviews
                                    </p>
                                </div>
                            </div>
                            ...
                            <div style={styles.ctaBannerLeft}>
                                ...
                                <div>
                                    <p style={{ ...styles.ctaBannerTitle, color: "#1A4A6A" }}>
                                        Your job is live! View matching candidates from the Applicants page.
                                    </p>
                                    <p style={{ ...styles.ctaBannerSub, color: "#1A6FA8" }}>
                                        Below is a preview of all registered candidates in the system.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    {stats && (
                        <div style={styles.statsRow}>
                            ...
                            <div style={styles.statCard}>
                                <span style={{ ...styles.statNum, color: "#1A7A4A" }}>
                                    {stats.new_this_week}
                                </span>
                                <span style={styles.statLabel}>New This Week</span>
                            </div>
                            <div style={styles.statCard}>
                                <span style={{ ...styles.statNum, color: "#A85A1A" }}>
                                    {stats.boosted_count}
                                </span>
                    ...
                    {loading ? (
                        ...
                        <div style={styles.emptyState}>
                            <svg width="64" height="64" viewBox="0 0 24 24"
                                fill="none" stroke="#CBD5E0" strokeWidth="1.2">
                                <circle cx="11" cy="11" r="8" />
                                <path d="M21 21l-4.35-4.35" />
                            </svg>
                            <p style={{
                                fontSize: 17, fontWeight: 600,
                                color: "#2D3748", margin: "8px 0 4px"
                            }}>
                            ...
                            <p style={{
                                ...
                                Use the filters above to find candidates by skill,
                                experience level, or city
                            </p>
                            ...
                            <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap", justifyContent: "center" }}>
                                {this.state.skills.slice(0, 6).map((skill) => (
                                    <button
                                        key={skill.id}
                                        onClick={() => this.handleFilter("skillId", String(skill.id))}
                                        style={{
                                            padding: "7px 16px", borderRadius: 20,
                                            border: "1px solid #BEE3F8", background: "#EBF8FF",
                                            color: "#2B6CB0", fontSize: 12, cursor: "pointer", fontWeight: 500,
                                        }}
                                    >
                                        💡 {skill.name}
                                    </button>
                        ...
                        <div style={styles.emptyState}>
                            <svg width="56" height="56" viewBox="0 0 24 24"
                                fill="none" stroke="#CBD5E0" strokeWidth="1.5">
                        ...
                        <>
                            ...
                            <div style={styles.grid}>
                                {candidates.map((candidate, idx) => (
                                    <CandidateCard
                                        ...
                                        candidate={candidate}
                                        index={idx}
                                        isLocked={!hasActiveJob || idx >= VISIBLE_FREE}
                                    />
                                ))}
                            </div>
                            ...
                            {(!hasActiveJob || candidates.length > VISIBLE_FREE) && (
                                <div style={styles.lockStrip}>
                                    ...
                                    <span>
                                        {hasActiveJob
                                            ? `${candidates.length - VISIBLE_FREE} more profiles locked — buy a package to access all`
                                            : `All ${total} profiles locked — post a job to unlock`}
                                    </span>
                                </div>
                            )}
                    ...
                    <div style={styles.bottomCta}>
                        <h3 style={styles.bottomCtaTitle}>
                            Want to hire from these candidates?
                        </h3>
                        <p style={styles.bottomCtaSub}>
                            Post a job, choose a package, and once reg_admin approves your job —
                            you can view matching candidates' full profiles and shortlist them for interviews.
                        </p>
                        <div style={styles.btnRow}>
                            <button style={styles.btnOutlinePurple} onClick={this.goToPackages}>
                                View Packages
                            </button>
                            <button style={styles.btnFillPurple} onClick={this.goToPostJob}>
                                Post a Job Now →
                            </button>
                        </div>
                    </div>
...
const styles = {
    ...
    cardSub: { fontSize: 12, color: "#718096", marginBottom: 10 },
