...
class PostBoxForm extends Component {
    ...
    try {
      ...
      const selectedSkills =
        job.skill_ids?.map((id) => {
          return (
            this.allSkills.find((skill) => skill.value === id) || {
              label: `Skill ${id}`,
              value: id,
            }
          );
  ...
  validateField = (name, value) => {
    ...
    if (name === "application_deadline") {
      ...
      if (deadline <= today)
        errors.deadline = "Application deadline must be greater than today.";
  ...
  validatemessage = () => {
    ...
    if (!values.skill_ids || values.skill_ids.length === 0)
      errors.skill_ids = "Please select at least one skill.";
    ...
    if (!values.no_of_positions)
      errors.no_of_positions = "Please enter number of positions.";
  ...
  handleSubmit = async (e) => {
    ...
    e.preventDefault();
    const { values, selectedCountry, selectedDistrict, selectedCity, jobId } =
      this.state;
    ...
    return (
      <Container fluid>
        ...
        <Form onSubmit={this.handleSubmit}>
          <Col>
            <h4 className="fw-bold" style={{ color: "#36565F" }}>
              Post a New Job
            </h4>
          </Col>
          <Card className="bg-light">
            <CardBody>
              <div
                className="alert alert-danger d-flex align-items-center gap-2 py-3 px-4 mb-4"
                role="alert"
              >
                <i className="las la-exclamation-circle fs-4"></i>
                <div className="fw-semibold"> All fields are mandatory</div>
              </div>
              <Row>
                {/* Job Title */}
                <Col md="12">
                ...
                <Col md="6">
                  <FormGroup>
                    ...
                    >
                      ...
                      <option value="medical_laboratory">
                        Medical Laboratory
                      </option>
              ...
              <div className="d-flex justify-content-end">
                <Button className="custom-progress-bar" type="submit">
                  Next
                </Button>
              </div>
              {showPricing && (
