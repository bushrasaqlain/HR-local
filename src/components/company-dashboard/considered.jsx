...
const StatusBadge = ({ status }) => {
  const map = {
    ...
    Considered: "bg-success text-white",
  };
  return <span className={`badge rounded-pill ${map[status] || "bg-secondary text-white"}`}>{status || "Considered"}</span>;
};
...
class Considered extends Component {
  state = {
    ...
    selectedStatus: "Considered", selectedCityId: "", searchFilters: {},
  ...
  fetchAllCandidates = async () => {
    ...
    try {
      ...
      this.setState({
        candidates, allApplicants: candidates, counts, selectedStatus: "Considered",
        splitViewActive: false, mobileDetailView: false,
        selectedCandidate: null, selectedCandidateId: null,
      });
    } catch (err) { console.error(err); }
  ...
  render() {
    ...
    return (
      <>
        <Head><title>Considered Candidates</title></Head>
                  ...
                  <div className="bg-white rounded-3 shadow p-3">
                    {current.length === 0 ? (
                      <div className="text-center py-5 text-muted">
                        <p className="fw-semibold mb-1">No considered candidates found</p>
