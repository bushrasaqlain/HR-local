...
const StatusBadge = ({ status }) => {
  const map = {
    ...
    Interview_Conducted: "bg-info text-white",
  };
  return <span className={`badge rounded-pill ${map[status] || "bg-secondary text-white"}`}>{status || "Scheduled"}</span>;
};
...
class Interview extends Component {
  state = {
    ...
    activeTab: "Interview_Scheduled", selectedStatus: "Interview_Scheduled",
    cities: [], selectedCandidate: null, selectedCandidateId: null,
    selectedCityId: "", searchFilters: {},
  ...
  openCandidatePage = (candidate) => {
    ...
    this.setState({
      ...
      splitViewActive: !mobile, mobileDetailView: mobile,
      showCandidateMessage: false,
    });
  };
  ...
  fetchAllCandidates = async () => {
    ...
    try {
      ...
      const counts = { all: candidates.length, pending: 0, shortlisted: 0, rejected: 0, approved: 0, conducted: 0 };
      candidates.forEach(a => {
        if (a.candidateStatus === "Pending") counts.pending++;
        if (a.candidateStatus === "Interview_Scheduled") counts.shortlisted++;
        if (a.candidateStatus === "Rejected") counts.rejected++;
        if (a.candidateStatus === "Approved") counts.approved++;
        if (a.candidateStatus === "Interview_Conducted") counts.conducted++;
      });

      this.setState({
        candidates, allApplicants: candidates, counts,
        selectedStatus: this.state.activeTab || "Interview_Scheduled",
        splitViewActive: false, mobileDetailView: false,
  ...
  render() {
    ...
    return (
      <>
        <Head><title>Interview Candidates</title></Head>
    ...
    {this.state.showJobDropdown && (
      ...
      >
        ...
        {this.state.postedJobs.filter(j =>
          ...
          <div className="px-3 py-2 small text-muted">No jobs found</div>
            ...
            {this.state.showFilters && (
              <>
                {/* Status Tabs */}
                <div className="bg-white rounded-3 shadow p-2 mb-3 d-flex gap-2">
                  {[
                    { label: "Interview Scheduled", value: "Interview_Scheduled", count: counts.shortlisted },
                    { label: "Interview Conducted", value: "Interview_Conducted", count: counts.conducted },
                  ].map(tab => (
                    <button
                      key={tab.value}
                      className={`btn btn-sm flex-fill d-flex align-items-center justify-content-center gap-2 rounded-2 fw-semibold ${this.state.activeTab === tab.value ? "text-white" : "text-secondary"}`}
                      style={{ background: this.state.activeTab === tab.value ? `linear-gradient(135deg, ${BRAND}, #4a7a8a)` : "transparent", border: "none" }}
                      onClick={() => this.setState({ activeTab: tab.value, selectedStatus: tab.value, currentPage: 1, splitViewActive: false, mobileDetailView: false, selectedCandidate: null, selectedCandidateId: null })}
                    >
                      {tab.label}
                      <span className={`badge rounded-pill ${this.state.activeTab === tab.value ? "bg-white text-dark" : "bg-light text-secondary"}`}>{tab.count}</span>
                    </button>
                  ))}
                </div>
                    ...
                    <div className="bg-white rounded-3 shadow p-3 overflow-auto" style={{ flex: "0 0 28%", maxHeight: "calc(100vh - 260px)" }}>
                      ...
                      <div className="d-flex flex-column gap-2">
                        {current.map(c => (
                          <div
                            key={c.id}
                            className={`d-flex align-items-center gap-2 p-2 rounded-2 cursor-pointer border ${c.id === selectedCandidateId ? "border-2" : "border-transparent"}`}
                          ...
                          <table className="table align-middle mb-0">
                            ...
                            <tbody>
                              {current.map(c => (
                                <tr
                                  key={c.id}
                                  className={`border-bottom ${c.id === selectedCandidateId ? "table-active" : ""}`}
                                ...
                                >
                                  ...
                                  {hasCandidateResponse && <>
                                    <td className="text-center">
                                      {c.candidateStatus === "Interview_Conducted" ? (
                                        <span className="badge rounded-pill bg-info-subtle text-info">Interview Done</span>
                                      ) : (
                                        <div>
                                    ...
                                    <td className="text-center">
                                      <div className="d-flex gap-1 justify-content-center">
                                        {c.candidateStatus === "Interview_Conducted" ? (
                                          <button className="btn btn-sm rounded-circle text-white" style={{ background: BRAND, width: 34, height: 34 }} onClick={e => { e.stopPropagation(); this.openCandidateMessage(c); }}><FaEnvelope size={14} /></button>
                                        ) : (
                                          <>
        ...
        {this.state.showConfirmRescheduleModal && (
          <div className="modal fade show d-block" style={{ background: "rgba(0,0,0,0.5)" }} onClick={() => this.setState({ showConfirmRescheduleModal: false })}>
            <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: 440 }} onClick={e => e.stopPropagation()}>
              <div className="modal-content">
                ...
                <div className="modal-body">
                  <p className="small">Confirm reschedule request from <strong>{this.state.selectedConfirmRescheduleCandidate?.full_name}</strong>?</p>
