...
const CandidateJobAlertsDropdown = ({ userId, apiBaseUrl, onViewJob }) => {
    ...
    const markOneRead = async (alertId) => {
        try {
            const token = sessionStorage.getItem("token");
            await fetch(
                `${apiBaseUrl}candidateProfile/job-alerts/${alertId}/read`,
                {
                    method: "PUT",
                    headers: { Authorization: `Bearer ${token}` },
                }
                    ...
                    <div style={{ maxHeight: "320px", overflowY: "auto" }}>
                        {alerts.length === 0 ? (
                            ...
                            >
                                ...
                                No job alerts yet.
                                <br />
                                <small>Set your job preferences to get notified!</small>
                            </div>
                        ) : (
                            alerts.map((alert) => (
                                <div
                                    key={alert.alert_id}
                                    onClick={() => {
                                        markOneRead(alert.alert_id);
                                        if (onViewJob) onViewJob(alert);
                                        setOpen(false);
                                    }}
                                    style={{
                                        display: "flex",
