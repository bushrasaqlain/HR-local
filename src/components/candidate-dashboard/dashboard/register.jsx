...
class CandidateRegisterForm extends Component {
  ...
  fetchAvailability = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await api.get(`${this.apiBaseUrl}candidate_availability/getavailability`, {
 ...
 handleCVUpload = async (e) => {
    ...
    this.setState({ cvUploading: true, formMessage: { type: "info", text: "Uploading CV..." } });
    try {
      const formData = new FormData();
      formData.append("resume", file);
      await api.post(`${this.apiBaseUrl}candidateProfile/candidate/upload-cv`, formData, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}`, "Content-Type": "multipart/form-data" },
      });
      this.setState({ cvUploadDone: true, cvUploading: false, formMessage: null });
    } catch (err) {
      this.setState({ cvUploading: false, formMessage: { type: "error", text: err.response?.data?.error || "Upload failed. Please try again." } });
    }
  };
