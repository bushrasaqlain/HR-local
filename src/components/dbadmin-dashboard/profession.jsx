...
class Profession extends Component {
  ...
  fetchProfession = async (
    ...
    try {
      const response = await axios.get(`${this.apiBaseUrl}getallprofessions`, {
        params: { page, limit: this.itemsPerPage, status },
      });
      this.setState({
        profession: response.data.professions || [],
        totalProfession: response.data.total || 0,
      });
    } catch (error) {
  ...
  handleSave = async () => {
    ...
    try {
      if (editId) {
        await api.put(`${this.apiBaseUrl}editprofession/${editId}`, {
          name: inputValue,
        });
        this.setState((prevState) => ({
          profession: prevState.profession.map((item) =>
            item.id === editId ? { ...item, name: inputValue } : item
          ),
        }));
      } else {
        await api.post(`${this.apiBaseUrl}addprofession`, { name: inputValue });
        this.fetchProfession(1);
      }
  ...
  handleDelete = async () => {
    ...
    try {
      await api.delete(`${this.apiBaseUrl}deleteprofession/${deleteId}`);
      toast.success(
        isActive === "active"
          ? "Inactivated successfully"
          : "Activated successfully"
      );
      this.setState({ showDeleteConfirm: false }, this.fetchProfession);
    } catch (error) {
  ...
  handleSearch = async (e) => {
    ...
    try {
      ...
      this.setState({
        profession: res.data.professions || [],
        totalProfession: res.data.total || 0,
      });
    } catch (error) {
  ...
  render() {
    ...
    const totalPages = Math.ceil(totalProfession / this.itemsPerPage);
    ...
    return (
      <React.Fragment>
        ...
        <h6 className="fw-bold mb-3">Profession List</h6>
        <div className="poppins-font">
          <Container fluid>
            <div className="Profession-header-section">
              <p
          ...
          >
            <Modal.Header closeButton style={{ paddingBottom: "0.25rem" }}>
              <Modal.Title style={{ fontSize: "1rem", marginBottom: "0" }}>
                {editId ? "Edit Profession" : "Add New Profession"}
              </Modal.Title>
            </Modal.Header>
