...
const Professions = () => {
  ...
  const totalPages = Math.ceil(totalprofessions / itemsPerPage);
     const [isActive, setIsActive] = useState("active");

  const apiBaseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;

  const fetchProfessions = async (page = currentPage, status = isActive) => {
    axios.get(`${apiBaseUrl}getallprofessions?page=${currentPage}&limit=15&status=${isActive}`)
      .then(response => {
        setPackageData(response.data.professions);
        setTotalprofessions(response.data.total);
      })
      .catch(error => {
 ...
 useEffect(() => {
  ...
  const handleSave = async () => {
    try {
      if (editId) {
        await api.put(`${apiBaseUrl}editprofession/${editId}`, { name: inputValue });
        setPackageData((prevData) => prevData.map((profession) => 
          profession.id === editId ? {...profession, name: inputValue}: profession));
      } else {
        await api.post(`${apiBaseUrl}addprofession`, { name: inputValue });
        fetchProfessions();
      }
      setShowModal(false);
      setInputValue("");
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`${apiBaseUrl}deleteprofession/${deleteId}`);
   ...
   const handlePageChange = async (page) => {
    ...
    try {
      const response = await axios.get(
        `${apiBaseUrl}search`, { params: { name: res.name, type: "Professions", page, limit: 15 } }
      );
  ...
  const fetchHistory = async (id) => {
    ...
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}dbadminhistory`, {
        params: {entity_type: "profession", entity_id: id}
      });
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      setPackageData(res.data.professions);
        setTotalprofessions(res.data.total);
    } catch (error) {
      ...
      {showModal && (
        <div className="Modal-outer-div">
          <div className="Modal-inner-div">
            <h4>{editId ? "Edit Profession" : "Add Profession"}</h4>
            <input
              type="text"
              value={inputValue}
      ...
      {showDeleteConfirm && (
        <div className="Modal-outer-div">
          <div className="Modal-inner-div">
            ...
            <p>Are you sure you want to {isActive} this Profession?</p>
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
