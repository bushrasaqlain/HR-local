...
const Degree = () => {
  ...
  const fetchdegreetypes = async (page = currentPage, status = isActive) => {
    axios.get(`${apiBaseUrl}getalldegrees?page=${currentPage}&limit=15&status=${isActive}`)
      .then(response => {
        setPackageData(response.data.degreetypes);
  ...
  const handleSave = async () => {
    try {
      if (editId) {
        await api.put(`${apiBaseUrl}editdegree/${editId}`, { name: inputValue });
        setPackageData((prevData) => prevData.map((item) => 
          item.id === editId ? {...item, name: inputValue}: item));
      } else {
        await api.post(`${apiBaseUrl}adddegree`, { name: inputValue });
        fetchdegreetypes();
      }
      setShowModal(false);
      setInputValue("");
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`${apiBaseUrl}deletedegree/${deleteId}`);
  ...
  const handlePageChange = async (page) => {
    ...
    try {
      const response = await axios.get(
        `${apiBaseUrl}search`, { params: { name: res.name, type: "Degree", page, limit: 15 } }
      );
  ...
  const handleSearch = async (e) => {
    setCurrentPage(1);
      ...
      try {
        const res = await axios.get(`${apiBaseUrl}getalldegrees`, {
          params: {
            name,
            search: value,
            status: isActive,
        ...
        setPackageData(res.data.degreetypes);
      ...
      {showModal && (
        <div className="Modal-outer-div">
          <div className="Modal-inner-div">
            <h4>{editId ? "Edit Degree" : "Add Degree"}</h4>
            <label>Name</label>
            <input
              type="text"
              value={inputValue}
      ...
      {showDeleteConfirm && (
        <div className="Modal-outer-div">
          <div className="Modal-inner-div">
            ...
            <p>Are you sure you want to {isActive} this Degree?</p>
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
