...
class Jobtype extends Component {
  ...
  fetchJobtype = async (
    ...
    try {
      const response = await axios.get(`${this.apiBaseUrl}getalljobtypes`, {
  ...
  handleSave = async () => {
    ...
    try {
      if (editId) {
        await api.put(`${this.apiBaseUrl}editjobtype/${editId}`, {
          name: inputValue,
        });
        this.setState((prevState) => ({
          jobtype: prevState.jobtype.map((item) =>
  ...
  handleSearch = async (e) => {
    ...
    try {
      ...
      this.setState({
        jobtype: res.data.jobtypes || [],
        totaljobtype: res.data.total || 0,
      });
    } catch (error) {
      console.error("Error searching jobtype:", error);
    }
  };
          ...
          >
            <Modal.Header closeButton style={{ paddingBottom: "0.25rem" }}>
              <Modal.Title style={{ fontSize: "1rem", marginBottom: "0" }}>
                {editId ? "Edit jobtype" : "Add New jobtype"}
              </Modal.Title>
            </Modal.Header>
