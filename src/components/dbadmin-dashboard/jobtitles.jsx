...
class JobTitle extends Component {
  ...
  fetchJobTitle = async (
    ...
    try {
      const response = await axios.get(`${this.apiBaseUrl}getAllJobTitles`, {
        params: { page, limit: this.itemsPerPage, status },
      });
      this.setState({
        jobtitle: response.data.jobtitles || [],
        totalJobTitle: response.data.total || 0,
      }, () => {
        // ✅ highlight check
        const lastHistoryType = sessionStorage.getItem("lastHistoryType");
        const lastHistoryId = sessionStorage.getItem("lastHistoryId");

        if (lastHistoryType === "jobtitle" && lastHistoryId) {
  ...
  handleExcelExport = () => {
    ...
    if (!jobtitle.length) {
      this.setState({ errorMessage: "No job titles available to export" });
      setTimeout(() => this.setState({ errorMessage: "" }), 3000);
      return;
    }

    // Map data for Excel
    const dataToExport = jobtitle.map((jobtitles) => ({
      "Name": jobtitles.name,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "jobtitle");

    // Write file
    XLSX.writeFile(workbook, "Jobtitle.xlsx");
  ...
  handleExcelImport = async (e) => {
    ...
    try {
      ...
      reader.onload = async (evt) => {
        ...
        await api.post(`${this.apiBaseUrl}addJobTitle`, {
          type: "csv",
          data: formattedData,
          userId,
        });

        this.setState({ successMessage: "Job title imported successfully" });
        setTimeout(() => this.setState({ successMessage: "" }), 3000);
        this.fetchJobTitle(1);
      };
  ...
  fetchHistory = async (id) => {
    ...
    try {
      const res = await axios.get(`${this.apiBaseUrl}dbadminhistory`, {
        params: { entity_type: "jobtitle", entity_id: id },
      });
  ...
  handleSave = async () => {
    ...
    try {
      if (editId) {
        await api.put(`${this.apiBaseUrl}editJobTitles/${editId}`, {
          name: inputValue,
        });
        this.setState((prevState) => ({
          jobtitle: prevState.jobtitle.map((item) =>
            item.id === editId ? { ...item, name: inputValue } : item
          ),
        }));
      } else {
        await api.post(`${this.apiBaseUrl}addjobtitle`, { name: inputValue });
        this.fetchJobTitle(1);
      }
      this.setState({
        showModal: false,
        inputValue: "",
        editId: null,
        successMessage: editId ? "Job type updated successfully!" : "Job Title added successfully!",
      });
      setTimeout(() => this.setState({ successMessage: "" }), 3000);
    } catch (error) {
  ...
  handleDelete = async () => {
    ...
    try {
      await api.delete(`${this.apiBaseUrl}deletejobtitle/${deleteId}`);
      this.setState(
        {
          showDeleteConfirm: false,
  ...
  handleSearch = async (e) => {
    ...
    try {
      ...
      this.setState({
        jobtitle: res.data.jobtitles || [],
        totaljobtype: res.data.total || 0,
      });
    } catch (error) {
      console.error("Error searching jobtitle:", error);
    }
  };
  ...
  render() {
    ...
    const totalPages = Math.ceil(totalJobTitle / this.itemsPerPage);
    ...
    return (
      <React.Fragment>
        ...
        <h6 className="fw-bold mb-3">Job Title List</h6>
                          ...
                          <td className="status text-center">
                            <div className="d-flex justify-content-center align-items-center gap-3">
                              ...
                              <button
                                className="icon-btn"
                                title="View History"
                                onClick={() => this.props.router.push(`/history/jobtitle/${item.id}`)}
                              >
          ...
          >
            <Modal.Header closeButton style={{ paddingBottom: "0.25rem" }}>
              <Modal.Title style={{ fontSize: "1rem", marginBottom: "0" }}>
                {editId ? "Edit jobtitle" : "Add New jobtitle"}
              </Modal.Title>
            </Modal.Header>
