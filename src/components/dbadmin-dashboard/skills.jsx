...
const Skills = () => {
  ...
  const fetchskills = async (page = currentPage, status = isActive) =>  {
    axios.get(`${apiBaseUrl}getallskills?page=${currentPage}&limit=15&status=${isActive}`)
      .then(response => {
 setPackageData(response.data.skills);
...
useEffect(() => {
  ...
  const handleSave = async () => {
    try {
      if (editId) {
        await api.put(`${apiBaseUrl}editskill/${editId}`, { name: inputValue });
        setPackageData((prevData)=> prevData.map((skill) =>
          skill.id === editId ? {...skill, name: inputValue}: skill));
      } else {
        await api.post(`${apiBaseUrl}addskill`, { name: inputValue });
        fetchskills();
      }
      setShowModal(false);
      setInputValue("");
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`${apiBaseUrl}deleteskill/${deleteId}`);
  ...
  const handlePageChange = async (page) => {
    ...
    try {
      const response = await axios.get(
        `${apiBaseUrl}search`, { params: { name: res.name, type: "Skills", page, limit: 15 } }
      );
      ...
      {showDeleteConfirm && (
        <div className="Modal-outer-div">
          <div className="Modal-inner-div">
            ...
            <p>Are you sure you want to {isActive} this Skill?</p>
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
