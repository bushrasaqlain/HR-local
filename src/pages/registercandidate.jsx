  ...
  constructor(props) {
    ...
    this.state = {
      ...
      formData: {
        ...
        license_number: "",
        country: "",
        otherPreferredCities: [], // <-- array of selected city IDs
        district: "",
        city: "",
...
fetchCandidateInfo = async () => {
  try {
    ...
    // 🔹 Load dependent dropdowns BEFORE setting formData
  ...
  values.education && values.education.length > 0
    ...
    : {
                            ...
                            value={
                              draft.institutes
                                ? {
                                    ...
                                    label: draft.institutes,
                                  }
                                : null
                            }
                            onChange={(opt) =>
                              setFieldValue(
                                "education.0.institutes",
                                opt?.label || "",
                              )
                            }
                          />
                      ...
                      <table className="table table-bordered">
                        ...
                        <tbody>
                          {values.education.slice(1).map((edu, i) => {
                            ...
                            // 🔹 Map ID → Name
                            const degreeName =
                              this.state.degreeFieldData.find(
                                (d) => String(d.id) === String(edu.degree),
                              )?.name || "";
                            ...
                            return (
                              <tr key={index}>
                                ...
                                <td>{edu.institutes}</td>
                                <td>
                                  <button
                                    type="button"
                                    className="btn btn-sm btn-warning me-2"
                                    onClick={() => {
                                      setFieldValue("education.0", edu);
                                      remove(index);
                                    }}
                                  >
                                    Edit
      ...
      case 4:
        return (
          <div>
            <h4>Step 4: Upload CV</h4>
            <input
              type="file"
              accept=".pdf,.docx"
              onChange={(e) => setFieldValue("resume", e.target.files[0])}
            />
            {values.resume && <span>{values.resume.name}</span>}
          </div>
        );
      case 5:
        return (
          <div>
            ...
            <FieldArray name="availability">
              {({ push, remove }) => (
                <div>
                  {values.availability.map((av, idx) => (
                    <div key={idx} className="mb-3 border p-3">
                      <Field
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      push({ day: "", shift: "", startTime: "", endTime: "" })
                    }
                  >
                    Add More
                  </button>
