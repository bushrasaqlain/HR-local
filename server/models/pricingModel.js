...
const createPackagesTable = () => {
  const packagetable = `
    CREATE TABLE IF NOT EXISTS packages (
      ...
      jobs_limit INT DEFAULT NULL,
      free_candidate_views INT DEFAULT 0,
      view_charge_type ENUM('percentage','fixed') DEFAULT 'percentage', 
view_charge_value DECIMAL(10,2) DEFAULT 0.1,
      package_type   ENUM('Company','Candidate','registration') DEFAULT 'Company',
      is_featured    TINYINT      DEFAULT 0,
      description    TEXT         NULL,
      pricing_model ENUM('package','per_job','credits_only'),
...
const validateByType = (body) => {
  ...
  if (!["Company", "Candidate", "registration"].includes(package_type)) {
    errors.push("Invalid package_type");
  }
...
const buildPayload = (body, currencyCode) => {
  ...
  return {
    ...
    duration_value: isRegistration ? null : Number(duration_value),

    price: isRegistration ? 0 : Number(price),

    currency: isRegistration ? null : currencyCode,
    currency_id: isRegistration ? null : currency_id,

    description: description ?? "Free trial for company registration",
    package_type,

    is_featured: is_featured ? 1 : 0,

    free_candidate_views: isCompany ? Number(free_candidate_views || 0) : null,
view_charge_type: isCompany ? (view_charge_type || "percentage") : null,
view_charge_value: isCompany ? Number(view_charge_value || 0) : null,
jobs_limit: isCompany ? Number(jobs_limit || 0) : null,
...
const addPackage = (req, res) => {
  ...
  const {
    ...
    price,
    package_type,
    pricing_model = "package", // ✅ NEW (default)
  } = req.body;
  ...
  connection.query(
    ...
    (err, currencyResult) => {
      ...
      // 🧠 Duplicate check only for paid packages
      let duplicateQuery = "";
      let params = [];
      ...
      const insertPackage = () => {
        ...
        const insertSql = `
          INSERT INTO packages
            (name, duration_unit, duration_value, price, currency, currency_id,
             description, package_type, is_featured,
             location_scope, pricing_model,
             jobs_limit, free_candidate_views,
        ...
        connection.query(insertSql, values, (err2, insertRes) => {
          if (err2) {
      ...
      if (isRegistration) {
        // ✅ Skip duplicate check for free/registration packages
        return insertPackage();
      }

      connection.query(duplicateQuery, params, (err, results) => {
        if (err) return res.status(500).json({ error: "Database error" });
