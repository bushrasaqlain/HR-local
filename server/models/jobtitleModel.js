...
`;
  ...
  connection.query(createTableQuery, (err) => {
    if (err) {
      return console.error("❌ Error creating jobtitles table:", err.message);
    }
    console.log("✅ jobTitles Table created successfully");
  });
};
...
const addJobTitle = (req, res) => {
  ...
  if (type === "csv") {
    ...
    const results = [];
    data.forEach((row) => {
      const jobTitleName = row.name?.trim();
      if (jobTitleName) {
        results.push([jobTitleName]);
      }
    });
    ...
    if (results.length === 0) {
      return res
        .status(400)
        .json({ error: "No valid job types found in CSV." });
    }

    const query = "INSERT INTO jobtitles (name) VALUES ?";
    connection.query(query, [results], (err, dbRes) => {
      if (err) {
        ...
        if (err.code === "ER_DUP_ENTRY") {
          return res
            .status(409)
            .json({ error: "Some job types already exist" });
        }
        return res.status(500).json({ error: "Database error" });
      }

      // Audit log for each inserted jobtitle
...
const deleteJobTitles = (req, res) => {
  ...
  const checkQuery = "SELECT * FROM jobtitles WHERE id = ?";
  connection.query(checkQuery, [id], (err, results) => {
    ...
    const updateQuery = "UPDATE jobtitles SET status = ? WHERE id = ?";
    connection.query(updateQuery, [newStatus, id], (err2) => {
      ...
      res
        .status(200)
        .json({ message: `Job type status updated to ${newStatus}` });
    });
  });
};
const editJobTitles = (req, res) => {
  ...
  connection.query(checkQuery, [id], (err, results) => {
    ...
    const updateQuery = "UPDATE jobtitles SET name = ? WHERE id = ?";
