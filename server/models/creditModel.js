...
const createCandidateUnlocksTable = () => {
  ...
  connection.query(sql, (err) => {
    if (err)
      return console.error("candidate_unlocks table error:", err.message);
        ...
        (err2, pkgRows) => {
          ...
          if (!pkgRows.length) {
            return res.status(403).json({
              error: "no_credits",
              message:
                "You do not have an active CV Credits package. Please purchase one to unlock profiles.",
            });
          }
          ...
          if (used >= total) {
            return res.status(403).json({
              error: "no_credits",
              message:
                "You have used all your credits. Please purchase a new pack to continue.",
            });
          }
                    ...
                    if (err4) {
                      return connection.rollback(() =>
                        res
                          ...
                          .json({
                            error:
                              "Failed to save unlock record. Please try again.",
                          }),
                      );
                    }
                      ...
                      if (commitErr) {
                        return connection.rollback(() =>
                          res
                            ...
                            .json({
                              error:
                                "Transaction commit failed. Please try again.",
                            }),
                        );
                      }
    ...
    (err, rows) => {
      ...
      if (!rows.length) {
        return res.status(403).json({
          ...
          message:
            "You have not unlocked this profile. Please use a credit to unlock it first.",
        });
      }
