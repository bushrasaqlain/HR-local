...
`;
    ...
    connection.query(createTableQuery, (err) => {
        if (err) return console.error("❌ Error creating cities table:", err.message);
...
const addCity = (req, res) => {
    ...
    if (type === "csv") {
        if (!district_id) return res.status(400).json({ error: "district_id is required for CSV import" });
        ...
        if (cities.length === 0) return res.status(400).json({ error: "No valid cities found in CSV." });

        const query = "INSERT INTO cities (district_id, name) VALUES ?";
        connection.query(query, [cities], (err, dbRes) => {
            if (err) {
                console.error("❌ Error inserting CSV cities:", err);
                if (err.code === "ER_DUP_ENTRY") {
                    return res.status(409).json({ error: "Some cities already exist in this district" });
                }
        ...
        if (!name || !district_id) {
            return res.status(400).json({ error: "City name and district_id are required" });
        }

        const checkQuery = "SELECT id FROM cities WHERE name = ? AND district_id = ?";
        connection.query(checkQuery, [name, district_id], (err, results) => {
            if (err) return res.status(500).json({ error: "Database error" });
            if (results.length > 0) return res.status(409).json({ message: "City already exists in this district" });

            const insertQuery = "INSERT INTO cities (district_id, name) VALUES (?, ?)";
            connection.query(insertQuery, [district_id, name], (err, insertResults) => {
                if (err) return res.status(500).json({ error: "Database error" });
                ...
                res.status(201).json({ success: true, message: "City added successfully", cityId: insertResults.insertId });
            });
        });
    }
}
...
const getCitiesByDistrict = (district_id, reqquery, callback) => {
 const districtId = district_id;
  ...
  connection.query(query, [districtId, `%${search}%`, limit, offset], (err, results) => {
    if (err) {
      console.error("❌ Error fetching cities:", err);
      return callback(err);
    }

    connection.query(countQuery, [districtId, `%${search}%`], (err2, countResult) => {
      if (err2) {
...
const editCity = (req, res) => {
 const { id } = req.params;
  ...
  connection.query(checkQuery, [id], (err, results) => {
    ...
    const updateQuery = "UPDATE cities SET name = ?, district_id = ? WHERE id = ?";
