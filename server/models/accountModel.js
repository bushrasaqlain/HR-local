...
const getAccountDetail = (req) => {
  return new Promise((resolve, reject) => {
    ...
    connection.query(sql, [userId], (err, results) => {
      ...
      let logoBase64 = null;
      if (results[0].logo)
        logoBase64 = Buffer.from(results[0].logo).toString("base64");
...
const getAccountType = (req, callback) => {
  ...
  connection.query(sql, [userId], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      return callback({
        status: 404,
        error: "User not found with the specified ID",
      });
    }
  });
};
...
const login = (req, res) => {
  try {
    ...
    const sql =
      "SELECT id, accountType, isActive FROM account WHERE email = ? AND password = ?";
    connection.query(sql, [email, password], (err, results) => {
      ...
      if (user.isActive !== "Active") {
        return res.json({
          success: false,
          error: "Admin has not granted permissions yet....",
        });
      }
      ...
      if (
        ...
        user.accountType === "db_admin" ||
        user.accountType === "reg_admin"
      ) {
...
const adminLogin = (req, res) => {
  ...
  connection.query(sql, [adminId, adminType], (err, results) => {
    ...
    if (results.length > 0) {
      ...
      return res
        .status(404)
        .json({ error: "Admin user not found with the specified ID" });
    }
  });
};
